...
export default function AbsensiGuruClient({ existingAbsensi }: { existingAbsensi: any }) {
  ...
  if (existingAbsensi || success) {
    const showAbsensi = existingAbsensi || { status, keterangan }
    return (
      <div className="bg-emerald-50 border border-emerald-200 p-10 rounded-2xl text-center max-w-xl mx-auto mt-10 shadow-sm">
        ...
        <p className="text-emerald-700 mb-4">Terima kasih, data kehadiran Anda hari ini telah tersimpan dalam sistem.</p>
        
        <div className="bg-white p-4 rounded-xl border border-emerald-100 mb-6 inline-block text-left mx-auto">
          <div className="text-sm text-slate-500 mb-1">Status Kehadiran Hari Ini:</div>
          <div className="font-bold text-emerald-700 text-lg flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
            {showAbsensi.status}
          </div>
          {showAbsensi.keterangan && (
            <div className="text-sm text-slate-600 mt-2 bg-slate-50 p-2 rounded border border-slate-100">
              "{showAbsensi.keterangan}"
            </div>
          )}
        </div>
