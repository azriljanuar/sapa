...
import PrintButton from "@/app/admin-jenjang/alumni/[id]/cetak-skl/print-button"

export default async function CetakSKLSantriPage({ params }: { params: { jenjangId: string } }) {
  const session = await getSession()
  if (!session || session.role !== "SANTRI") redirect("/login")

  const jenjangId = parseInt(params.jenjangId)
  if (isNaN(jenjangId)) notFound()
  ...
  if (!alumniData.isAlumni) {
    return <div className="p-6 text-center text-red-500">Anda bukan alumni di jenjang ini.</div>
  }
...
<div style="margin-top: 50px; text-align: right;">
  ...
  // TODO: Get tahun ajaran when they graduated. 
  const tahunAjaran = new Date().getFullYear().toString()
  ...
  const contentHtml = templateStr
    ...
    .replace(/{{TAHUN_LULUS}}/g, new Date().getFullYear().toString())
